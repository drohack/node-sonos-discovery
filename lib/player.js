...
function Player(roomName, descriptorUrl, uuid, discovery) {
  ...
  function updateTransportState(data) {
    ...
    //Set playlistName
    if (data['r:EnqueuedTransportURIMetaData']) {
    	var playlistName = parseMetaDataXML(data['r:EnqueuedTransportURIMetaData'], _this)[0].title;
    	_this.state.playlistName = playlistName;
    }
